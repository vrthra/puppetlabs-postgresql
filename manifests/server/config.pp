...
class postgresql::server::config {
  ...
  if ($ensure == 'present' or $ensure == true) {
    ...
    if(versioncmp($postgresql::params::version, '8.2') >= 0) {
      $extras_filename = 'postgresql_puppet_extras.conf'
      $extras_path = "`dirname ${postgresql_conf_path}`/${extras_filename}"
      exec { 'create_postgresql_conf_path':
        command => "touch ${extras_path}",
        path    => '/usr/bin:/bin',
        unless  => "[ -f ${extras_path} ]"
      }

      postgresql::config_entry { 'include':
        value   => $extras_filename,
